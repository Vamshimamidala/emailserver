...
router.post('/register', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        user = new User({
            ...
            isVerified: false, // Add an `isVerified` field to your User schema
        });
...
router.post('/login', async (req, res) => {
 ...
 router.post('/forgot', async (req, res) => {
    ...
    try {
        ...
        const durationSeconds = 600; // For 10 minute
 const token = jwt.sign({ userId: user._id }, JWT_SECRET, { expiresIn: durationSeconds });
        
        const resetLink = `https://reset2.onrender.com?token=${token}`;
...
router.post('/switch-account', authMiddleware, async (req, res) => {
    const { email, password } = req.body;
    ...
    try {
      ...
      // Compare the new account password with the stored password
      const isMatch = await bcrypt.compare(password, user.password);
      if (!isMatch) {
        return res.status(400).json({ msg: 'Invalid credentials' });
      }
      ...
      res.json({ token, msg: 'Switched to new account successfully' });
    } catch (err) {
